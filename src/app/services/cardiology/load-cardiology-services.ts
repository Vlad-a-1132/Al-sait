/**
 * Живые услуги и цены для кардиологии: те же вызовы fetchPriceServices / fetchPriceCategories,
...
/** Совпадает с выбором ветки на /prices?cat=5025 (КАРДИОЛОГ) */
export const CARDIOLOGY_PRICE_CATEGORY_ID = 5025;
...
export async function loadCardiologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(CARDIOLOGY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatCardiologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

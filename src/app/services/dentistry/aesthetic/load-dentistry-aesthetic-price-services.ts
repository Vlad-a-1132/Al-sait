/**
 * Живые услуги эстетической стоматологии: профилактика заболеваний полости рта (ветка 4730).
 */
...
export const DENTISTRY_AESTHETIC_PRICE_SECTIONS = [
  { categoryId: 4730, title: "Профилактика заболеваний полости рта" },
] as const;

export type DentistryAestheticPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryAestheticPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryAestheticPriceSections(): Promise<DentistryAestheticPriceSection[]> {
  ...
  const out: DentistryAestheticPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_AESTHETIC_PRICE_SECTIONS) {

/**
 * Живые услуги брекет-систем (ветка 4746).
 */
...
export const DENTISTRY_BRACES_PRICE_SECTIONS = [
  { categoryId: 4746, title: "Брекет-системы" },
] as const;

export type DentistryBracesPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryBracesPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryBracesPriceSections(): Promise<DentistryBracesPriceSection[]> {
  ...
  const out: DentistryBracesPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_BRACES_PRICE_SECTIONS) {

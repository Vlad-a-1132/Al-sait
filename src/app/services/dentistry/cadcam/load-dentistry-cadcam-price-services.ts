/**
 * Живые услуги CAD/CAM-технологий (ветка 4740).
 */
...
export const DENTISTRY_CADCAM_PRICE_SECTIONS = [
  { categoryId: 4740, title: "CAD/CAM-технологии" },
] as const;

export type DentistryCadcamPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryCadcamPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryCadcamPriceSections(): Promise<DentistryCadcamPriceSection[]> {
  ...
  const out: DentistryCadcamPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_CADCAM_PRICE_SECTIONS) {

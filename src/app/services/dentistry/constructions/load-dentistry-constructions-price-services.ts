/**
 * Живые услуги по изготовлению и починке стоматологических конструкций (ветка 4741).
 */
...
export const DENTISTRY_CONSTRUCTIONS_PRICE_SECTIONS = [
  { categoryId: 4741, title: "Услуги по изготовлению и починке стоматологических конструкций" },
] as const;

export type DentistryConstructionsPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryConstructionsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryConstructionsPriceSections(): Promise<DentistryConstructionsPriceSection[]> {
  ...
  const out: DentistryConstructionsPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_CONSTRUCTIONS_PRICE_SECTIONS) {

/**
 * Живые услуги и цены: приёмы врача-стоматолога (ветка категории 4725).
 */
...
/** Приемы врача-стоматолога, терапевта, зубного врача (id 4725) */
export const DENTISTRY_CONSULTATION_PRICE_CATEGORY_ID = 4725;
...
export async function loadDentistryConsultationPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(DENTISTRY_CONSULTATION_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    DENTISTRY_CONSULTATION_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatConsultationPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

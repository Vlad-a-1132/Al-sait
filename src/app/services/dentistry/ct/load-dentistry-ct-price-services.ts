/**
 * Живые услуги компьютерной томографии челюстно-лицевой области (ветка 4723).
 */
...
export const DENTISTRY_CT_PRICE_SECTIONS = [
  { categoryId: 4723, title: "Компьютерная томография челюстно-лицевой области" },
] as const;

export type DentistryCtPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryCtPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryCtPriceSections(): Promise<DentistryCtPriceSection[]> {
  ...
  const out: DentistryCtPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_CT_PRICE_SECTIONS) {

/**
 * Живые услуги удаления зубных отложений / профилактика (ветка 4730).
 */
...
export const DENTISTRY_DEPOSITS_PRICE_SECTIONS = [
  { categoryId: 4730, title: "Профилактика заболеваний полости рта" },
] as const;

export type DentistryDepositsPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryDepositsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryDepositsPriceSections(): Promise<DentistryDepositsPriceSection[]> {
  ...
  const out: DentistryDepositsPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_DEPOSITS_PRICE_SECTIONS) {

/**
 * Живые услуги по удалению зубов и сопутствующим операциям (ветка 4733).
 */
...
export const DENTISTRY_EXTRACTION_PRICE_SECTIONS = [
  { categoryId: 4733, title: "Удаление зубов, прочие операции и услуги" },
] as const;

export type DentistryExtractionPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryExtractionPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryExtractionPriceSections(): Promise<DentistryExtractionPriceSection[]> {
  ...
  const out: DentistryExtractionPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_EXTRACTION_PRICE_SECTIONS) {

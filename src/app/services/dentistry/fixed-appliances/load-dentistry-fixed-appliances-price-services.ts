/**
 * Живые услуги несъёмных ортодонтических аппаратов (ветка 4748).
 */
...
export const DENTISTRY_FIXED_APPLIANCES_PRICE_SECTIONS = [
  { categoryId: 4748, title: "Несъемные аппараты" },
] as const;

export type DentistryFixedAppliancesPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryFixedAppliancesPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryFixedAppliancesPriceSections(): Promise<DentistryFixedAppliancesPriceSection[]> {
  ...
  const out: DentistryFixedAppliancesPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_FIXED_APPLIANCES_PRICE_SECTIONS) {

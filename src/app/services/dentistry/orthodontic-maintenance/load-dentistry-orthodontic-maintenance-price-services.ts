/**
 * Живые услуги по обслуживанию ортодонтических аппаратов (ветка 4749).
 */
...
export const DENTISTRY_ORTHODONTIC_MAINTENANCE_PRICE_SECTIONS = [
  { categoryId: 4749, title: "Услуги по обслуживанию ортодонтических аппаратов" },
] as const;

export type DentistryOrthodonticMaintenancePriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryOrthodonticMaintenancePriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryOrthodonticMaintenancePriceSections(): Promise<DentistryOrthodonticMaintenancePriceSection[]> {
  ...
  const out: DentistryOrthodonticMaintenancePriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_ORTHODONTIC_MAINTENANCE_PRICE_SECTIONS) {

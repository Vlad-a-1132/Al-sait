/**
 * Живые услуги ортодонтии: приёмы ортодонта (4743) и спец. лечебно-диагностические услуги (4744).
 */
...
export const DENTISTRY_ORTHODONTICS_PRICE_SECTIONS = [
  { categoryId: 4743, title: "Приемы врача-стоматолога-ортодонта" },
  { categoryId: 4744, title: "Специальные лечебно-диагностические услуги" },
] as const;

export type DentistryOrthodonticsPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryOrthodonticsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryOrthodonticsPriceSections(): Promise<DentistryOrthodonticsPriceSection[]> {
  ...
  const out: DentistryOrthodonticsPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_ORTHODONTICS_PRICE_SECTIONS) {

/**
 * Живые услуги и цены: приёмы врача-стоматолога детского (ветка категории 4751).
 */
...
/** Приемы врача-стоматолога детского (id 4751) */
export const PEDIATRIC_DENTISTRY_RECEPTION_CATEGORY_ID = 4751;
...
export async function loadPediatricDentistryReceptionPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(PEDIATRIC_DENTISTRY_RECEPTION_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    PEDIATRIC_DENTISTRY_RECEPTION_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatPediatricDentistryPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

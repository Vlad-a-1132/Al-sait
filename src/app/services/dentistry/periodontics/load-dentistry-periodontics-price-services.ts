/**
 * Живые услуги пародонтологии: общие лечебно-диагностические услуги (ветка 4721).
 */
...
export const DENTISTRY_PERIODONTICS_PRICE_SECTIONS = [
  { categoryId: 4721, title: "Общие лечебно-диагностические услуги" },
] as const;

export type DentistryPeriodonticsPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryPeriodonticsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryPeriodonticsPriceSections(): Promise<DentistryPeriodonticsPriceSection[]> {
  ...
  const out: DentistryPeriodonticsPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_PERIODONTICS_PRICE_SECTIONS) {

/**
 * Живые услуги челюстно-лицевой хирургии (ветка 4735) — страница пластической хирургии полости рта.
 */
...
export const DENTISTRY_PLASTIC_PRICE_SECTIONS = [
  { categoryId: 4735, title: "Челюстно-лицевая хирургия" },
] as const;

export type DentistryPlasticPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryPlasticPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryPlasticPriceSections(): Promise<DentistryPlasticPriceSection[]> {
  ...
  const out: DentistryPlasticPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_PLASTIC_PRICE_SECTIONS) {

/**
 * Живые услуги ортопедического направления: протезирование зубов (ветка 4739).
 */
...
export const DENTISTRY_PROSTHETICS_PRICE_SECTIONS = [
  { categoryId: 4739, title: "Протезирование зубов" },
] as const;

export type DentistryProstheticsPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryProstheticsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryProstheticsPriceSections(): Promise<DentistryProstheticsPriceSection[]> {
  ...
  const out: DentistryProstheticsPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_PROSTHETICS_PRICE_SECTIONS) {

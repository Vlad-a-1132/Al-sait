/**
 * Живые услуги ортопедической стоматологии: приёмы врача-стоматолога-ортопеда (ветка 4737).
 */
...
export const DENTISTRY_PROSTHODONTICS_PRICE_SECTIONS = [
  { categoryId: 4737, title: "Приемы врача-стоматолога-ортопеда" },
] as const;

export type DentistryProsthodonticsPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryProsthodonticsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryProsthodonticsPriceSections(): Promise<DentistryProsthodonticsPriceSection[]> {
  ...
  const out: DentistryProsthodonticsPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_PROSTHODONTICS_PRICE_SECTIONS) {

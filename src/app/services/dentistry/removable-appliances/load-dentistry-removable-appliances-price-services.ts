/**
 * Живые услуги съёмных ортодонтических аппаратов (ветка 4747).
 */
...
export const DENTISTRY_REMOVABLE_APPLIANCES_PRICE_SECTIONS = [
  { categoryId: 4747, title: "Съемные аппараты" },
] as const;

export type DentistryRemovableAppliancesPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryRemovableAppliancesPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryRemovableAppliancesPriceSections(): Promise<DentistryRemovableAppliancesPriceSection[]> {
  ...
  const out: DentistryRemovableAppliancesPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_REMOVABLE_APPLIANCES_PRICE_SECTIONS) {

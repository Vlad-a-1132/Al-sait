/**
 * Живые услуги "Укрепление тканей зуба" из ветки 4730,
 * но выводим только выбранные услуги по коду.
 */
...
export const DENTISTRY_STRENGTHENING_PRICE_SECTIONS = [
  { categoryId: 4730, title: "Профилактика заболеваний полости рта" },
] as const;

const DENTISTRY_STRENGTHENING_INCLUDED_CODES = new Set<string>([
  "А11.07.012",
  "А11.07.012.001",
  "А11.07.012.500",
  "А16.07.057.504",
...
export type DentistryStrengtheningPriceSection = {
  sectionKey: string;
  sectionTitle: string;
  services: PriceServiceDisplay[];
};
...
export function formatDentistryStrengtheningPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryStrengtheningPriceSections(): Promise<DentistryStrengtheningPriceSection[]> {
  ...
  const out: DentistryStrengtheningPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_STRENGTHENING_PRICE_SECTIONS) {
    const allowedCategoryIds = collectDescendantCategoryIds(categoryId, childrenMap);
    ...
    let ordered = flattenServicesInPricesBranchOrder(
      categoryId,
      childrenMap,
      servicesByCategory,
      allowedCategoryIds,
    ...
    ordered = ordered.filter((s) =>
      DENTISTRY_STRENGTHENING_INCLUDED_CODES.has(normalizeServiceCode(s.code))
    );
    ordered.sort((a, b) =>
      normalizeServiceCode(a.code).localeCompare(normalizeServiceCode(b.code), "ru")
    );
    out.push({
      sectionKey: String(categoryId),
      sectionTitle: title,
      services: ordered,
    });
  }

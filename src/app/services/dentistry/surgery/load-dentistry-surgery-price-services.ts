/**
 * Живые услуги хирургической стоматологии: три ветки прайса (консультации, удаление/операции, ЧЛХ).
 */
...
export const DENTISTRY_SURGERY_PRICE_SECTIONS = [
  { categoryId: 4732, title: "Приемы врача-стоматолога-хирурга" },
  { categoryId: 4733, title: "Удаление зубов, прочие операции и услуги" },
  { categoryId: 4735, title: "Челюстно-лицевая хирургия" },
] as const;

export type DentistrySurgeryPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistrySurgeryPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistrySurgeryPriceSections(): Promise<DentistrySurgeryPriceSection[]> {
  ...
  const out: DentistrySurgeryPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_SURGERY_PRICE_SECTIONS) {

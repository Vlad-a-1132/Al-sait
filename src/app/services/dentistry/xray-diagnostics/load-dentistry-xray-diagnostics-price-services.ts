/**
 * Живые услуги диагностики:
 * - КТ челюстно-лицевой области (ветка 4723)
 * - Рентгенология и прочие сложные диагностические исследования (ветка 4722)
 */
...
export const DENTISTRY_XRAY_DIAGNOSTICS_PRICE_SECTIONS = [
  { categoryId: 4723, title: "Компьютерная томография челюстно-лицевой области" },
  { categoryId: 4722, title: "Рентгенология и прочие сложные диагностические исследования" },
] as const;

export type DentistryXrayDiagnosticsPriceSection = {
  sectionKey: string;
  sectionTitle: string;
...
export function formatDentistryXrayDiagnosticsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;
...
export async function loadDentistryXrayDiagnosticsPriceSections(): Promise<DentistryXrayDiagnosticsPriceSection[]> {
  ...
  const out: DentistryXrayDiagnosticsPriceSection[] = [];

  for (const { categoryId, title } of DENTISTRY_XRAY_DIAGNOSTICS_PRICE_SECTIONS) {
    ...
    const ordered = flattenServicesInPricesBranchOrder(
      categoryId,
      childrenMap,
      servicesByCategory,
      allowedCategoryIds,

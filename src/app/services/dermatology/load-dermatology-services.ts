/**
 * Живые услуги и цены для дерматологии: те же вызовы fetchPriceServices / fetchPriceCategories,
...
/** Совпадает с выбором ветки на /prices?cat=5013 (ДЕРМАТОЛОГ) */
export const DERMATOLOGY_PRICE_CATEGORY_ID = 5013;
...
export async function loadDermatologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(DERMATOLOGY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatDermatologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

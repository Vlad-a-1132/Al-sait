/**
 * Живые услуги и цены для диетологии: загрузка из price-api по ветке категории.
 */
...
/** ДИЕТОЛОГ (id 5009) */
export const DIETETICS_PRICE_CATEGORY_ID = 5009;
...
export async function loadDieteticsPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(DIETETICS_PRICE_CATEGORY_ID, childrenMap);
...
export function formatDieteticsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

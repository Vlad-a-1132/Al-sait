/**
 * Живые услуги и цены для эндокринологии: загрузка из price-api по ветке категории.
 */
...
/** ЭНДОКРИНОЛОГ (id 5026) */
export const ENDOCRINOLOGY_PRICE_CATEGORY_ID = 5026;
...
export async function loadEndocrinologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(ENDOCRINOLOGY_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    ENDOCRINOLOGY_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatEndocrinologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

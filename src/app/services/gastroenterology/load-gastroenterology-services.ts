/**
 * Живые услуги и цены для блока гастроэнтерологии: те же вызовы fetchPriceServices /
...
/** Совпадает с выбором ветки на /prices?cat=5008 (ГАСТРОЭНТЕРОЛОГ) */
export const GASTROENTEROLOGY_PRICE_CATEGORY_ID = 5008;
...
export async function loadGastroenterologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(GASTROENTEROLOGY_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    GASTROENTEROLOGY_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatGastroenterologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

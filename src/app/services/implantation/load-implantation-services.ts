/**
 * Живые услуги и цены для блока имплантации: те же вызовы fetchPriceServices /
...
/** Совпадает с выбором ветки на /prices?cat=4734 */
export const IMPLANTATION_PRICE_CATEGORY_ID = 4734;
...
/**
 * Тот же порядок обхода, что у ветки на `/prices?cat=4734`:
...
function flattenServicesInPricesBranchOrder(
  ...
  function walk(id: number): void {
    ...
    out.push(...(servicesByCategory.get(id) ?? []));
...
export async function loadImplantationPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(IMPLANTATION_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    IMPLANTATION_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatImplantationPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

/**
 * Живые услуги и цены для лазерной косметологии: загрузка из price-api по ветке категории.
 *
 * Ветка: КОСМЕТОЛОГ (id 5046) + все дочерние категории (всего в ветке: 136).
 */
...
export const LASER_COSMETOLOGY_PRICE_CATEGORY_ID = 5046;
...
export async function loadLaserCosmetologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(LASER_COSMETOLOGY_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    LASER_COSMETOLOGY_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatLaserCosmetologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

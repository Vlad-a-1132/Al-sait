/**
 * Живые услуги и цены для маммологии: те же вызовы fetchPriceServices / fetchPriceCategories,
 * что использует страница `/prices`, с фильтром по ветке категории.
 *
 * Ветка: ОНКОЛОГ (id 5023) — на странице /services/mammology выводим прайс из БД.
 */
...
/** Совпадает с выбором ветки на /prices?cat=5023 (ОНКОЛОГ) */
export const MAMMOLOGY_PRICE_CATEGORY_ID = 5023;
...
export async function loadMammologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(MAMMOLOGY_PRICE_CATEGORY_ID, childrenMap);

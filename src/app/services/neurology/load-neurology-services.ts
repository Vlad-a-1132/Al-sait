/**
 * Живые услуги и цены для неврологии: те же вызовы fetchPriceServices / fetchPriceCategories,
...
/** Совпадает с выбором ветки на /prices?cat=5024 (НЕВРОЛОГ) */
export const NEUROLOGY_PRICE_CATEGORY_ID = 5024;
...
export async function loadNeurologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(NEUROLOGY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatNeurologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

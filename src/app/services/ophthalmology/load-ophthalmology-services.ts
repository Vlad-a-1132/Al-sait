/**
 * Живые услуги и цены для офтальмологии: те же вызовы fetchPriceServices / fetchPriceCategories,
...
/** Совпадает с выбором ветки на /prices?cat=5012 (ОФТАЛЬМОЛОГ) */
export const OPHTHALMOLOGY_PRICE_CATEGORY_ID = 5012;
...
export async function loadOphthalmologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(OPHTHALMOLOGY_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    OPHTHALMOLOGY_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatOphthalmologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

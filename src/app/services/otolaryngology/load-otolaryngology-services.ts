/**
 * Живые услуги и цены для ЛОР: те же вызовы fetchPriceServices / fetchPriceCategories,
 * что использует страница `/prices`, с фильтром по ветке категории.
 */
...
/** Совпадает с выбором ветки на /prices?cat=5011 (ОТОРИНОЛАРИНГОЛОГ/СУРДОЛОГ-ОТОРИНОЛАРИНГОЛОГ) */
export const OTOLARYNGOLOGY_PRICE_CATEGORY_ID = 5011;
...
export async function loadOtolaryngologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(OTOLARYNGOLOGY_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    OTOLARYNGOLOGY_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatOtolaryngologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

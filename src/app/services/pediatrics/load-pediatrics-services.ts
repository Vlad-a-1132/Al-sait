/**
 * Живые услуги и цены для педиатрии: загрузка из price-api по ветке категории.
 */
...
/** ПЕДИАТР (id 5007) */
export const PEDIATRICS_PRICE_CATEGORY_ID = 5007;
...
export async function loadPediatricsPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(PEDIATRICS_PRICE_CATEGORY_ID, childrenMap);
...
export function formatPediatricsPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

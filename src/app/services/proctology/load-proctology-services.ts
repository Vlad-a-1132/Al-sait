/**
 * Живые услуги и цены для проктологии: загрузка из price-api по ветке категории.
 */
...
/** КОЛОПРОКТОЛОГ (id 5028) */
export const PROCTOLOGY_PRICE_CATEGORY_ID = 5028;
...
export async function loadProctologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(PROCTOLOGY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatProctologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

/**
 * Живые услуги и цены для рентгенологии: загрузка из price-api по ветке категории.
 */
...
/** РЕНТГЕНОГРАФИЯ (id 5031) */
export const RENTGENOLOGY_PRICE_CATEGORY_ID = 5031;
...
export async function loadRentgenologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(RENTGENOLOGY_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    RENTGENOLOGY_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatRentgenologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

/**
 * Живые услуги и цены для сурдологии: загрузка из price-api по ветке категории.
 *
 * Ветка: ОТОРИНОЛАРИНГОЛОГ/СУРДОЛОГ-ОТОРИНОЛАРИНГОЛОГ (id 5011).
 */
...
export const SURDOLOGY_PRICE_CATEGORY_ID = 5011;
...
export async function loadSurdologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(SURDOLOGY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatSurdologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

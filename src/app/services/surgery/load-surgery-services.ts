/**
 * Живые услуги и цены для хирургии: загрузка из price-api по ветке категории.
 */
...
/** ХИРУРГ (id 5029) */
export const SURGERY_PRICE_CATEGORY_ID = 5029;
...
export async function loadSurgeryPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(SURGERY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatSurgeryPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

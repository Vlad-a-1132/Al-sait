/**
 * Живые услуги и цены для терапии: загрузка из price-api по ветке категории.
 */
...
/** ТЕРАПЕВТ (id 5006) */
export const THERAPY_PRICE_CATEGORY_ID = 5006;
...
export async function loadTherapyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(THERAPY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatTherapyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

/**
 * Живые услуги и цены для травматологии: загрузка из price-api по ветке категории.
 */
...
/** ТРАВМАТОЛОГ-ОРТОПЕД (id 5030) */
export const TRAUMATOLOGY_PRICE_CATEGORY_ID = 5030;
...
export async function loadTraumatologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(TRAUMATOLOGY_PRICE_CATEGORY_ID, childrenMap);
  ...
  return flattenServicesInPricesBranchOrder(
    TRAUMATOLOGY_PRICE_CATEGORY_ID,
    childrenMap,
    servicesByCategory,
    allowedCategoryIds,
    idToName
...
export function formatTraumatologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

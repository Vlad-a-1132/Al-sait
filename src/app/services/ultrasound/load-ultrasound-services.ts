/**
 * Живые услуги и цены для блока УЗИ: те же вызовы fetchPriceServices /
...
/** Совпадает с выбором ветки на /prices?cat=5032 */
export const ULTRASOUND_PRICE_CATEGORY_ID = 5032;
...
/**
 * Услуги той же ветки категории, что и на странице прайса с ?cat=5032
 * (категория 5032 и все потомки в дереве).
 */
export async function loadUltrasoundPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(ULTRASOUND_PRICE_CATEGORY_ID, childrenMap);
...
export function formatUltrasoundPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

/**
 * Живые услуги и цены для урологии: загрузка из price-api по ветке категории.
 */
...
/** УРОЛОГ (id 5027) */
export const UROLOGY_PRICE_CATEGORY_ID = 5027;
...
export async function loadUrologyPriceServices(): Promise<PriceServiceDisplay[]> {
  ...
  const allowedCategoryIds = collectDescendantCategoryIds(UROLOGY_PRICE_CATEGORY_ID, childrenMap);
...
export function formatUrologyPriceLabel(s: PriceServiceDisplay): string {
  const p = s.price;

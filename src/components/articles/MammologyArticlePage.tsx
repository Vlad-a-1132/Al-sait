...
import { MAMMOLOGY_ARTICLE_PAGE_DEFS } from "@/data/mammology-article-pages";

const SERVICE_URL = "/services/mammology";
const HERO = "/images/images allergoly/mamalogy.webp";
const PHONE_LINK = "tel:+74952554450";
const APPOINTMENTS_URL = "https://online.altamed-c.ru/";
const BLOG_URL = "/blog";

export function makeMammologyArticleMetadata(slug: string): Metadata {
  const def = MAMMOLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
  ...
  return {
    ...
    openGraph: {
      ...
      locale: "ru_RU",
      type: "article",
      images: [{ url: "https://altamed-c.ru/images/images%20allergoly/mamalogy.webp", alt: def.h1 }],
    },
    alternates: { canonical: url },
  };
}

export default function MammologyArticlePage({ slug }: { slug: string }) {
  const def = MAMMOLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
    return (
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-start">
            <div>
              ...
              <div className="mt-6 text-sm text-gray-600">
                Полезные ссылки:{" "}
                <Link href={`${SERVICE_URL}#mammology-doctors`} className="text-emerald-700 font-medium hover:underline">
                  врачи
                </Link>
                ,{" "}
                <Link href={`${SERVICE_URL}#mamm-prices`} className="text-emerald-700 font-medium hover:underline">
                  цены
                </Link>
                ,{" "}
      ...
      <section className="py-10 sm:py-12 bg-gray-50/60 border-t border-gray-100">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Запишитесь к маммологу в Одинцово</h2>
            <p className="text-gray-700 leading-relaxed mb-6">
              Если вас беспокоят боль, уплотнение, выделения из соска, изменения кожи или формы молочной железы, либо вы хотите пройти профилактический
              осмотр, запишитесь на консультацию маммолога в клинику «Альтамед-С». Врач проведёт осмотр, оценит жалобы и подскажет, какие обследования
              действительно нужны.
            </p>
          ...
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {def.relatedSlugs.map((s) => {
                const r = MAMMOLOGY_ARTICLE_PAGE_DEFS[s];
                if (!r) return null;
                return (
                  <Link
                    key={s}
                  ...
                  >
                    <div className="text-xs text-gray-500 mb-2">Маммология</div>

...
import { NEUROLOGY_ARTICLE_PAGE_DEFS } from "@/data/neurology-article-pages";

const SERVICE_URL = "/services/neurology";
const HERO = "/images/images allergoly/nevrolog.webp";
const PHONE_LINK = "tel:+74952554450";
const APPOINTMENTS_URL = "https://online.altamed-c.ru/";
const BLOG_URL = "/blog";

export function makeNeurologyArticleMetadata(slug: string): Metadata {
  const def = NEUROLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
  ...
  return {
    ...
    openGraph: {
      ...
      locale: "ru_RU",
      type: "article",
      images: [{ url: "https://altamed-c.ru/images/images%20allergoly/nevrolog.webp", alt: def.h1 }],
    },
    alternates: { canonical: url },
  };
}

export default function NeurologyArticlePage({ slug }: { slug: string }) {
  const def = NEUROLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
    return (
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-start">
            <div>
              ...
              <div className="mt-6 text-sm text-gray-600">
                Полезные ссылки:{" "}
                <Link href={`${SERVICE_URL}#neurology-doctors`} className="text-emerald-700 font-medium hover:underline">
                  врачи
                </Link>
                ,{" "}
      ...
      <section className="py-10 sm:py-12 bg-gray-50/60 border-t border-gray-100">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Запишитесь к неврологу в Одинцово</h2>
            <p className="text-gray-700 leading-relaxed mb-6">
              Если вас беспокоят головные боли, мигрень, головокружение, боль в спине или шее, онемение, слабость, нарушения сна или последствия травм и
              неврологических заболеваний, запишитесь на консультацию невролога в клинику «Альтамед-С». Врач проведёт осмотр, оценит симптомы и подскажет,
          ...
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {def.relatedSlugs.map((s) => {
                const r = NEUROLOGY_ARTICLE_PAGE_DEFS[s];
                if (!r) return null;
                return (
                  <Link
                    key={s}
                  ...
                  >
                    <div className="text-xs text-gray-500 mb-2">Неврология</div>

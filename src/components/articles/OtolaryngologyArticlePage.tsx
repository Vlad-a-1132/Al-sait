...
import { OTOLARYNGOLOGY_ARTICLE_PAGE_DEFS } from "@/data/otolaryngology-article-pages";

const SERVICE_URL = "/services/otolaryngology";
const HERO = "/images/images allergoly/otolaryngologia.webp";
const PHONE_LINK = "tel:+74952554450";
const APPOINTMENTS_URL = "https://online.altamed-c.ru/";
const BLOG_URL = "/blog";

export function makeOtolaryngologyArticleMetadata(slug: string): Metadata {
  const def = OTOLARYNGOLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
  ...
  return {
    ...
    openGraph: {
      ...
      locale: "ru_RU",
      type: "article",
      images: [{ url: "https://altamed-c.ru/images/images%20allergoly/otolaryngologia.webp", alt: def.h1 }],
    },
    alternates: { canonical: url },
  };
}

export default function OtolaryngologyArticlePage({ slug }: { slug: string }) {
  const def = OTOLARYNGOLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
    return (
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-start">
            <div>
              ...
              <div className="mt-6 text-sm text-gray-600">
                Полезные ссылки:{" "}
                <Link href={`${SERVICE_URL}#lor-doctors`} className="text-emerald-700 font-medium hover:underline">
                  врачи
                </Link>
                ,{" "}
                <Link href={`${SERVICE_URL}#services`} className="text-emerald-700 font-medium hover:underline">
                  цены
                </Link>
                ,{" "}
            ...
            <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-lg ring-1 ring-black/5 max-h-[320px] lg:max-h-none">
              <Image src={HERO} alt={def.h1} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 560px" priority unoptimized />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 sm:py-12 bg-gray-50/60 border-t border-gray-100">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Запишитесь к ЛОР-врачу в Одинцово</h2>
            <p className="text-gray-700 leading-relaxed mb-6">
              Если вас беспокоят боль в ухе, заложенность носа, длительный насморк, боль в горле, пробки в миндалинах, снижение слуха, осиплость голоса
              или другие симптомы со стороны уха, горла и носа, запишитесь на консультацию отоларинголога в клинику «Альтамед-С». Врач проведёт осмотр и
              подскажет, какие обследования или процедуры действительно нужны.
            </p>
            <div className="flex flex-col sm:flex-row flex-wrap gap-3">
              ...
              <Link href={`${SERVICE_URL}#services`} className="inline-flex justify-center items-center px-5 py-3.5 text-emerald-700 font-medium hover:underline min-h-[44px]">
                Прайс ЛОР
              </Link>
            </div>
          </div>
          ...
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {def.relatedSlugs.map((s) => {
                const r = OTOLARYNGOLOGY_ARTICLE_PAGE_DEFS[s];
                if (!r) return null;
                return (
                  <Link
                    key={s}
                  ...
                  >
                    <div className="text-xs text-gray-500 mb-2">ЛОР</div>

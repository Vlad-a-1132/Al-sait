...
import { PEDIATRICS_ARTICLE_PAGE_DEFS } from "@/data/pediatrics-article-pages";

const SERVICE_URL = "/services/pediatrics";
const HERO = "/images/yslugi/Pediatrician.webp";
const PHONE_LINK = "tel:+74952554450";
const APPOINTMENTS_URL = "https://online.altamed-c.ru/";
const BLOG_URL = "/blog";

const SERVICE_PRICES = [
  { title: "Первичный приём педиатра", price: "3 260 ₽" },
  { title: "Повторный приём педиатра", price: "2 840 ₽" },
  { title: "Профилактический приём", price: "1 050 ₽" },
  { title: "Диспансерный приём", price: "2 420 ₽" },
  { title: "Вакцинация", price: "530 ₽" },
] as const;

export function makePediatricsArticleMetadata(slug: string): Metadata {
  const def = PEDIATRICS_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
...
export default function PediatricsArticlePage({ slug }: { slug: string }) {
  const def = PEDIATRICS_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
    return (
  ...
  return (
    <article className="min-h-screen bg-white">
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-start">
            <div>
              ...
              <div className="flex flex-col sm:flex-row flex-wrap gap-3 mt-6">
                ...
                <Link
                  href={SERVICE_URL}
                  className="inline-flex justify-center items-center px-5 py-3.5 text-emerald-700 font-medium hover:underline min-h-[44px]"
                >
                  Услуги педиатрии
                </Link>
              </div>
      ...
      <section className="py-10 sm:py-12 bg-gray-50/60 border-t border-gray-100">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Услуги педиатрии в «Альтамед‑С»</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
              {SERVICE_PRICES.map((s) => (
                <div key={s.title} className="rounded-xl border border-gray-200 bg-white p-4">
                  ...
                  <div className="text-emerald-700 font-bold mt-1">{s.price}</div>
                </div>
              ))}
            </div>
            <p className="text-xs sm:text-sm text-gray-600 mb-6">
              Цены справочные. Актуальную стоимость и возможность записи уточняйте по телефону{" "}
              <a className="text-emerald-700 font-medium hover:underline" href={PHONE_LINK}>
                +7 (495) 255‑44‑50
              </a>
              .
            </p>
          ...
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {def.relatedSlugs.map((s) => {
                const r = PEDIATRICS_ARTICLE_PAGE_DEFS[s];
                if (!r) return null;
                return (
                  <Link key={s} href={`${BLOG_URL}/${s}`} className="rounded-2xl border border-gray-200 bg-white p-5 hover:border-emerald-200 hover:shadow-md transition">
                    <div className="text-xs text-gray-500 mb-2">Педиатрия</div>
      ...
      <footer className="border-t border-gray-100 bg-gray-50/80">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 text-sm text-gray-600">
          <p className="font-medium text-gray-800">Стаченкова Светлана Валериевна</p>
          <p className="mt-1">Врач-педиатр, врач-остеопат. Стаж: 29 лет.</p>
        </div>
      </footer>
    </article>
  );

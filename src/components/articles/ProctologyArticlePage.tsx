...
import { PROCTOLOGY_ARTICLE_PAGE_DEFS } from "@/data/proctology-article-pages";

const SERVICE_URL = "/services/proctology";
const HERO = "/images/images allergoly/proktolog.webp";
const PHONE_LINK = "tel:+74952554450";
const APPOINTMENTS_URL = "https://online.altamed-c.ru/";
const BLOG_URL = "/blog";

const SERVICE_LIST = [
  "Приём колопроктолога первичный",
  "Приём колопроктолога повторный",
  "Ректороманоскопия",
  "Латексное лигирование геморроидального узла",
  "Склеротерапия геморроидальных узлов",
  "Тромбэктомия из геморроидального узла",
  "Иссечение анальной трещины",
  "Иссечение анальной бахромки",
  "Лечение свищей и парапроктита по показаниям",
  "Лечение копчикового хода по показаниям",
] as const;

export function makeProctologyArticleMetadata(slug: string): Metadata {
  const def = PROCTOLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
  ...
  return {
    ...
    openGraph: {
      ...
      locale: "ru_RU",
      type: "article",
      images: [{ url: "https://altamed-c.ru/images/images%20allergoly/proktolog.webp", alt: def.h1 }],
    },
    alternates: { canonical: url },
  };
}

export default function ProctologyArticlePage({ slug }: { slug: string }) {
  const def = PROCTOLOGY_ARTICLE_PAGE_DEFS[slug];
  if (!def) {
    return (
      ...
      <section className="py-10 sm:py-12 bg-gray-50/60 border-t border-gray-100">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Услуги проктологии в «Альтамед‑С»</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
              {SERVICE_LIST.map((t) => (
                <div key={t} className="rounded-xl border border-gray-200 bg-white p-4">
                  <div className="text-sm font-semibold text-gray-900 leading-snug">{t}</div>
                </div>
              ))}
            </div>
          ...
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {def.relatedSlugs.map((s) => {
                const r = PROCTOLOGY_ARTICLE_PAGE_DEFS[s];
                if (!r) return null;
                return (
                  <Link key={s} href={`${BLOG_URL}/${s}`} className="rounded-2xl border border-gray-200 bg-white p-5 hover:border-emerald-200 hover:shadow-md transition">
                    <div className="text-xs text-gray-500 mb-2">Проктология</div>
